...
local function Position_MicroMenuButtons()
	if (UnitLevel("player") < SHOW_SPEC_LEVEL) then
		-- Ensure the talent button is shown, since it's hidden on the default UI
...
local function ActivateLongBar()
	...
	-- MainMenuBar Position  (moves all actions)
...
function Update_StatusBars()
	...
	elseif MainMenuExpBar:IsShown() and MainMenuExpBar:IsShown() then
		if shortBarActive then
...
local function PlayerLogin()
	...
	Update_ActionBars()
	-- Update_PixelPerfectInterfaceWarnings()
	UpdateBagSpace()
end
local f = CreateFrame("Frame")
...
-- Most information about the game world should now be available to the UI
-- If this is an interface reload rather than a fresh log in,
-- talent information should also be available
local function PlayerEnteringWorld()

end
local f = CreateFrame("Frame")
f:RegisterEvent("PLAYER_ENTERING_WORLD")
f:SetScript("OnEvent", PlayerEnteringWorld)
...
local function PlayerEnteredCombat()
	...
	-- Disable ActionBar checkboxes
	InterfaceOptionsActionBarsPanelBottomLeft:Disable()
	_G[InterfaceOptionsActionBarsPanelBottomLeft:GetName() .. "Text"]:SetText("Show Bottom Left ActionBar (can't toggle in combat)")
	InterfaceOptionsActionBarsPanelBottomRight:Disable()
	_G[InterfaceOptionsActionBarsPanelBottomRight:GetName() .. "Text"]:SetText("Show Bottom Right ActionBar (can't toggle in combat)")
end
local f = CreateFrame("Frame")
f:RegisterEvent("PLAYER_REGEN_DISABLED")
f:SetScript("OnEvent", PlayerEnteredCombat)

local ShowAsExperienceBar_Text = ReputationDetailMainScreenCheckBoxText:GetText()

local function PlayerLeftCombat()
	-- Enable "Show as Experience Bar" checkbox
	ReputationDetailMainScreenCheckBox:Enable()
	ReputationDetailMainScreenCheckBoxText:SetTextColor(1, 0.82, 0)
	ReputationDetailMainScreenCheckBoxText:SetText(ShowAsExperienceBar_Text)

	-- Enable ActionBar checkboxes
	InterfaceOptionsActionBarsPanelBottomLeft:Enable()
	_G[InterfaceOptionsActionBarsPanelBottomLeft:GetName() .. "Text"]:SetText("Show Bottom Left ActionBar")
	InterfaceOptionsActionBarsPanelBottomRight:Enable()
	_G[InterfaceOptionsActionBarsPanelBottomRight:GetName() .. "Text"]:SetText("Show Bottom Right ActionBar")
